...
describe('POST /api/bulk/verify', () => {
  ...
  describe('Response Structure', () => {
    ...
    it('should handle internal server errors gracefully', async () => {
      // Send malformed JSON to trigger error handling (central error handler returns 500)
      const response = await request(app)
        .post('/api/bulk/verify')
        .set('X-API-Key', ENTERPRISE_KEY)
      ...
      expect(response.body).toMatchObject({ code: 'INTERNAL_ERROR', message: expect.any(String) })
    })
  })
})
